...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'shortstack':
				dialogue = CoolUtil.coolTextFile(Paths.txt('shortstack/shortstackDialogue'));
		}
		...
		switch (curStage)
		{
			...
			case 'arcade': //Raine Week
				var arc:FlxSprite = new FlxSprite(-600, -275).loadGraphic(Paths.image('raineArc', 'raine'));
				arc.active = false;
				arc.setGraphicSize(Std.int(arc.width * 0.78));
				add(arc);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns' | 'shortstack':
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (chartingMode)
			{
				...
				function shortOutro1(?dialogueBox:DialogueBox):Void
				{
					camFollow.y = 450;
					camFollow.x = 450;
					add(dialogueBox);
			...
			function shortOutro2(?dialogueBox:DialogueBox):Void
			{
					camFollow.x = gf.getMidpoint().x;
					camFollow.y = gf.getMidpoint().y;
					new FlxTimer().start(2.5, function(swagTimer:FlxTimer)
					{
						camFollow.y = 450;
						camFollow.x = 450;
						var doof:DialogueBox = new DialogueBox(false, CoolUtil.coolTextFile(Paths.txt('shortstack/shortEndPart2')));
						doof.scrollFactor.set();
						doof.finishThing = function()
						{
							...
							FlxG.sound.playMusic(Paths.music('raineMenu'));
							transitioning = true;
					}
						add(doof);
						doof.cameras = [camHUD];
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						case 'shortstack':
							trace('LOADING NO SONG WORK PLEASE I LOVE YOU');
							var doof:DialogueBox = new DialogueBox(false, CoolUtil.coolTextFile(Paths.txt('shortstack/shortEndPart1')));
							doof.scrollFactor.set();
							doof.cameras = [camHUD];
							shortOutro1(doof);
							canPause = false;
				...
				if (PlayState.SONG.song.toLowerCase() != 'shortstack')
				{
					MusicBeatState.switchState(new StoryMenuState());
				}
